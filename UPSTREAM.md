...
| `src/logic/events/RunningApplicationsEvents.swift` + `src/logic/Applications.swift` (`removeRunningApplications`, `317a485bcb090bf2b29e3f78872218f0099e1d62`) | `Engine/WindowStore.swift` | KVO launch and termination tracking; removal matches `NSRunningApplication` with `isEqual` when a departed process PID is no longer usable |
