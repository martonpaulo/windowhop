...
| `src/ui/settings-window/tabs/controls/ControlsTab.swift` (`.commandKeyAboveTab` as `kVK_ANSI_Grave`), `src/logic/Preferences.swift` (`keyAboveTabDependingOnInputSource`) | `WindowHopKit/ShortcutSpec.swift` (`KeyCode.grave`) + `Input/EventTap.swift` | the key macOS binds ⌘` to is key code 50 on every layout, labelled by the current input source; AltTab steps back only with Shift, so `` ` `` stepping back in a session is WindowHop's rule (WindowHop issue #135) |
| `src/logic/BackgroundWork.swift` | `Engine/BackgroundWork.swift` | dedicated run-loop threads, AX off the main thread |
| `src/logic/events/RunningApplicationsEvents.swift` + `src/logic/Applications.swift` (`removeRunningApplications`, `317a485bcb090bf2b29e3f78872218f0099e1d62`) | `Engine/WindowStore.swift` + `WindowHopKit/AppDeparture.swift` | KVO launch and termination tracking; removal matches the departed `NSRunningApplication` with `isEqual`, never by its PID, and without an `isTerminated` gate (WindowHop issue #136). WindowHop adds one guard upstream lacks: an app still listed in `runningApplications` is kept |
