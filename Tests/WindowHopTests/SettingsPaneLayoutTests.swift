...
extension SharedAppState {
    ...
    final class SettingsPaneLayoutTests {
        ...
        @Test func everyPaneRendersIntoTheSameCanvas() {
            _ = NSApplication.shared
            let canvas = CGSize(
                width: DesignTokens.settingsPaneWidth,
                height: DesignTokens.settingsPaneHeight)
            #expect(SettingsPane.allCases.count > 1)
        ...
        @Test func appearancePaneKeepsItsCanvasInEveryMode() {
            _ = NSApplication.shared
            ...
            for mode in AppearanceMode.allCases {
                ...
                #expect(
                    SettingsPane.appearance.makeViewController(isolated.settingsDependencies)
                        .sizeThatFits(in: unbounded) == canvas, "the Appearance pane resizes in \(mode.rawValue)")
            }
        }
