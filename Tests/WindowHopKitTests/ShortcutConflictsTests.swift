...
final class ShortcutConflictsTests: XCTestCase {
    ...
    private func assess(_ shortcut: PersistentShortcut,
                        switcher: ShortcutSpec = .commandTab,
                        current: PersistentShortcut? = nil,
                        system: [PersistentShortcut] = [],
                        labels: KeyLabelSource = ANSIKeyLabels()) -> PersistentShortcut.CaptureAssessment {
    ...
    func testEveryStandardCommandIsRejectedWithItsName() {
        for command in ShortcutConflicts.standardCommands {
            ...
            XCTAssertEqual(assess(shortcut),
                           .reject(.standardApplicationCommand(chord: shortcut.displayString,
                                                              command: command.name)),
                           command.name)
        }
        XCTAssertEqual(PersistentShortcut.ValidationError
    ...
    func testCommandsAreMatchedByTheLayoutsCharacter() {
        ...
        XCTAssertEqual(assess(selectAll, labels: Self.azerty),
                       .reject(.standardApplicationCommand(chord: "⌘A", command: "Select All")))
        XCTAssertEqual(assess(quit, labels: Self.azerty),
                       .reject(.standardApplicationCommand(chord: "⌘Q", command: "Quit")))
        XCTAssertEqual(assess(PersistentShortcut(keyCode: 13, modifiers: [.maskCommand]), labels: Self.azerty),
    ...
    func testUntranslatedKeysFallBackToTheANSICharacter() {
        ...
        XCTAssertEqual(ShortcutConflicts.standardCommand(
    ...
    func testForceQuitChordsAreReservedByMacOS() {
        for reserved in ShortcutConflicts.reservedByMacOS {
            XCTAssertEqual(assess(reserved, system: [reserved]),
                           .reject(.reservedByMacOS(chord: reserved.displayString)))
        }
        XCTAssertEqual(ShortcutConflicts.reservedByMacOS.map(\.displayString), ["⌥⌘⎋", "⌥⇧⌘⎋", "⌃⌥⇧⌘⎋"])
        XCTAssertEqual(PersistentShortcut.ValidationError.reservedByMacOS(chord: "⌥⌘⎋").explanation,
    ...
    func testReRecordingTheStoredChordIsAccepted() {
        ...
        XCTAssertEqual(assess(Self.commandSpace, current: Self.commandSpace, system: [Self.commandSpace]),
                       .accept)
    }
    ...
    func testLoadTimeRulesStillComeFirst() {
        ...
        XCTAssertEqual(assess(optionTab, switcher: .optionTab, current: optionTab),
                       .reject(.conflictsWithSwitcherShortcut),
                       "the current chord does not excuse a switcher conflict")
        XCTAssertEqual(assess(PersistentShortcut(keyCode: 12, modifiers: [.maskShift])),
