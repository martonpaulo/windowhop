...
public enum TapMode {
    ...
    case off
    /// idle: consume only the trigger chord when at least one window is eligible
    case watching
    /// session open: consume the handled keys (Tab, arrows, Escape, Return, Delete)
    case session
...
public final class EventTap {
    ...
    private func handle(type: CGEventType, event: CGEvent) -> Unmanaged<CGEvent>? {
        ...
        case .off, .passthrough:
            return Unmanaged.passUnretained(event)
        case .watching:
            return handleWhileWatching(type, event, holdModifier, eligibleCount)
        case .session:
            return handleDuringSession(type, event, holdModifier)
        }
    }

    private func handleWhileWatching(_ type: CGEventType, _ event: CGEvent,
                                     _ holdModifier: CGEventFlags, _ eligibleCount: Int) -> Unmanaged<CGEvent>? {
        guard type == .keyDown,
              event.getIntegerValueField(.keyboardEventKeycode) == KeyCode.tab,
              event.flags.contains(holdModifier),
              // only Shift may accompany the chord; Cmd-Opt-Tab etc. stay native
              event.flags.isDisjoint(with: otherModifiers(than: holdModifier)) else {
            return Unmanaged.passUnretained(event)
        }
    ...
    private func handleDuringSession(_ type: CGEventType, _ event: CGEvent,
                                     _ holdModifier: CGEventFlags) -> Unmanaged<CGEvent>? {
        if type == .flagsChanged {
            ...
            if !event.flags.contains(holdModifier) {
                post(.modifierReleased)
            }
            return Unmanaged.passUnretained(event)
        }
        let keyCode = event.getIntegerValueField(.keyboardEventKeycode)
        guard let handled = sessionEvent(for: keyCode, flags: event.flags) else {
    ...
    private func sessionEvent(for keyCode: Int64, flags: CGEventFlags) -> SwitcherInputEvent? {
        switch keyCode {
        case KeyCode.tab:
