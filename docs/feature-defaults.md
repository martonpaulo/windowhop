...
| About attribution and website | Shown | No | Application metadata, centralized in `ProjectLinks`; no persistence or reset entry. Settings › About is the one About surface: the app menu's About WindowHop opens that pane instead of AppKit's standard About panel, and its copyright footer, the only developer credit, is the bundle's `NSHumanReadableCopyright` read through `WindowHopKit/AppVersion` (omitted in unbundled development builds), with the AltTab credit as text and no link to the AltTab repository (#123, #132). "Report an Issue…" (About and Help) opens the public bug-report form prefilled through `ProjectLinks.issueReport` with the version, build, ISO release date when present and macOS version only; the person reviews and submits it. |
...
| `` ` `` steps back in a session | On | No | One valid outcome that matches native ⌘Tab, where ⌘` moves the selection back (#135). Key code 50 on every layout, labelled by the current input source; ⇧Tab and ← remain. Consumed only while a session is open, so ⌘` outside a session stays the system's. No persistence or reset entry. |
...
| Switching guide in Settings | Shown | No | Explanatory copy derived from the current switcher shortcut, Open WindowHop shortcut and Enable state through `SwitchingGuide` and `ShortcutFormatter`, with no state of its own: the status line in General, the hint under each shortcut and the session key rows in Shortcuts (#121). No persistence, migration or reset entry, and it never touches first-launch state or visibility choices. |
