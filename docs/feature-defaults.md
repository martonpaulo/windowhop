...
| Window Previews size | Medium | Yes | Appearance pane; typed `UserDefaults` (`previewSize`): Small keeps the original 188 × 118 pt card on every display, Medium grows cards until four rows fill the panel height (AltTab Medium), Large until three (AltTab Large); growing cards keep three per row, the app badge scales with the canvas, and mirrored panels share the most constrained display's size. No migration — a missing or invalid value takes Medium; resets to Medium. Disabled in App Icons, whose proportions stay fixed. |
